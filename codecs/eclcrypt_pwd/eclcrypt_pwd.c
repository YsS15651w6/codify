...
static void trie_insert(TrieNode *root, const char *encoded,
...
void shuffle_map(const char **shuffled_map, int seed) {
  // Initialize shuffled_map with NULLs
  for (int i = 0; i < 256; i++) {
    shuffled_map[i] = NULL;
  }

  // Collect valid indices and values from the base map
  int valid_indices[256];
  const char *valid_values[256];
  int count = 0;

  for (int i = 0; i < 256; i++) {
    if (map[i]) {
      valid_indices[count] = i;
      valid_values[count] = map[i];
      count++;
    }
  }
  ...
  // Assign shuffled values back to the valid indices
  for (int i = 0; i < count; i++) {
    shuffled_map[valid_indices[i]] = valid_values[i];
  }
}
...
char *eclcrypt_pwd_decode(const char *input, const char *password) {
  ...
  for (int i = 0; i < 256; i++) {
    if (shuffled_map[i])
      trie_insert(root, shuffled_map[i], (unsigned char)i);
  }
